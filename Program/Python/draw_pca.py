...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for gene in tqdm.tqdm(selected_genes):
            ...
            matplotlib.pyplot.xlabel(f"PC-1 ({input_adata.uns['pca']['variance_ratio'][0] * 100:.1f}%)")
            matplotlib.pyplot.ylabel(f"PC-2 ({input_adata.uns['pca']['variance_ratio'][1] * 100:.1f}%)")
        ...
        for i, variance in tqdm.contrib.tenumerate(input_adata.uns["pca"]["variance_ratio"], start=1):
            i = i[0] + 1
        ...
        matplotlib.pyplot.xticks(range(len(input_adata.uns["pca"]["variance_ratio"])), ["" for _ in input_adata.uns["pca"]["variance_ratio"]])
        matplotlib.pyplot.ylim(bottom=0, top=1.25 * max(input_adata.uns["pca"]["variance_ratio"]))
